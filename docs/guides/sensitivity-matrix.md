...
> **🌏 语言 / Language**: 中文摘要。完整规范（含 `normal` / `private` / `restricted` 三档判定流程、`actorMaxSensitivity` 派生规则、`auth-context.ts` 的 `resolveAuthorization()` 伪代码、与 Profile（`core` / `extended` / `admin`）的组合矩阵）请参阅 [`sensitivity-matrix.en.md`](./sensitivity-matrix.en.md)。  
> **Summary in Chinese**. For the full spec (`normal` / `private` / `restricted` decision flow, `actorMaxSensitivity` derivation, `resolveAuthorization()` pseudocode, profile × sensitivity matrix) see [`sensitivity-matrix.en.md`](./sensitivity-matrix.en.md).  
