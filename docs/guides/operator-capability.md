...
> **🌏 语言 / Language**: 中文摘要。完整规范（含错误码、状态机、跨平台细节、详细示例）请参阅 [`operator-capability.en.md`](./operator-capability.en.md)。  
> **Summary in Chinese**. For the full spec (error codes, state machine, cross-platform details, exhaustive examples) see [`operator-capability.en.md`](./operator-capability.en.md).  
