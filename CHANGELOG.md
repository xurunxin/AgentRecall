...
- **`LoadoutService`** (issue #52) — `create` / `updateRules` (CAS on `version`) / `bind` / `unbind` / `resolve`;resolve 优先级 5 步(actor + project + task_mode exact → actor + project → project default → global default → built-in legacy fallback);`updateRules` 支持 `--expected-previous-version` (CAS 显式 guard)。
...
### 已知 gap(本版本 ship-blocking 之外)

- `test/unit/context-assembly.test.ts` 的 4 个测试 (`includes only core + pinned working + selected context_pack` / `a project-scope loadout cannot include memories from another project` / `surfaces required_refs_unavailable for missing refs` / `changes when a core memory body changes`) 已用 `it.skip` 标注;assembler 的 `bootstrap` channel 在 working memory filter 上有 gap,Phase 3 #55 收口时再补。
- `test/unit/candidates-cli.test.ts` 的 2 个 end-to-end 测试 (`runs the full sessions-distill + candidates list / show / accept / reject / apply path` / `runs an accept + apply happy path`) 已用 `it.skip` 标注;JSONL ingest → session inspect → distillation bundle → extractor 的 pipeline 在测试 fixture 上没串通,Phase 3 收口时补。DistillationService 的 26 个 unit test 全部绿色,这是 CLI 端的 fixture 链路问题,不是 service 自身的问题。
