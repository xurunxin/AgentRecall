...
describe("release evidence aggregator", () => {
  ...
  it("fails when a fragment is missing", () => { const x = setup(); rmSync(join(x.fragments, "release-evidence-fragment-matrix-v113-win32-x64.json")); const result = aggregate(x); expect(result.status).not.toBe(0); expect(result.stderr).toContain("MISMATCHED_PLATFORMS"); });
