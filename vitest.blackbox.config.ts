...
export default defineConfig({
  test: {
    ...
    include: [
      ...
      // v1.1.6 follow-up D1: re-include
      // mcp-stdio-idle.test.ts. The v1.1.5
      // cap-bounded wait was rewritten to a
      // sentinel-wait; the test now passes on the
      // release-candidate orchestrator (the
      // `[lifecycle] idle-sentinel` line is the
      // assertion, the 5 s `setTimeout` is just
      // a CI-safety backstop).
